...
We begin by proving that for each $A \in \mathcal{B}$ with finite measure, the characteristic function $\chi_A$ can be approximated, in the $L^p$ norm, by functions in $C_c(X)$. Let $\epsilon > 0$. By \PMlinkescapetext{inner and outer regularity}{OuterRegular} of $\mu$, we know there exist an open set $U$ and a compact set $K$ such that $K \subset A \subset U$ and
\begin{align*}
